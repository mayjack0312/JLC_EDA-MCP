# Run API Gateway — 常见问题与进阶使用

[English](./README.en.md) · [简体中文](./README.md) · [FAQ English](./FAQ.en.md)
...
- [从零开始使用教程](#从零开始使用教程)
  ...
  - [4. 首次启动 OpenCode 并连接模型](#4-首次启动-opencode-并连接模型)
  - [5. 安装 EasyEDA API Skill](#5-安装-easyeda-api-skill)
  - [6. 在嘉立创EDA 专业版中安装本扩展](#6-在嘉立创eda-专业版中安装本扩展)
  - [7. 打开 EDA 并等待扩展连接](#7-打开-eda-并等待扩展连接)
  - [8. 在 OpenCode 中发出启动指令](#8-在-opencode-中发出启动指令)
  - [9. 验证是否真的已经可以调用 EDA](#9-验证是否真的已经可以调用-eda)
  - [10. 一个最推荐的新手完整操作顺序](#10-一个最推荐的新手完整操作顺序)
- [连接失败时怎么排查](#连接失败时怎么排查)
- [适合直接复制给 OpenCode 的示例提示词](#适合直接复制给-opencode-的示例提示词)
- [这套方案里每个组件分别做什么](#这套方案里每个组件分别做什么)
- [补充说明：开发者本地调试模式](#补充说明开发者本地调试模式)
- [本地构建与开发](#本地构建与开发)
...
如果这一步失败，先回到 [第 3 节](#3-安装-opencode) 重新安装 OpenCode。
...
如果命令安装仍然失败，可以回到前面的 [5.2 如果命令安装失败，如何手动下载并安装 Skill](#52-如果命令安装失败如何手动下载并安装-skill) 按步骤手动安装。
...
如果你的电脑需要走代理才能访问外网，`npx clawhub@latest install` 和 `opencode-ai` 自身的网络请求可能也会被代理影响。常见症状包括：
...
- Windows：在「系统设置 → 网络和 Internet → 代理」中确认代理已启用
- 终端需要识别系统代理，或者使用 `HTTP_PROXY` / `HTTPS_PROXY` 环境变量显式设置
- 某些公司网络还会拦截 `ws://127.0.0.1` 的握手；如果 EDA 内提示 `Handshake failed: unexpected service`，说明 127.0.0.1 上的某个进程被错误地识别为 Bridge，请关闭其他占用 49620-49629 端口的程序再试

### 11.8 端口冲突

如果端口 `49620-49629` 已经被其他程序占用（常见的有 IDE Live Share、调试代理、其它服务），扩展会持续扫不到 Bridge Server。
...
如果发现非预期占用，关闭对应程序后回到 EDA 点击 **API Gateway** → **重新连接**。

---

## 适合直接复制给 OpenCode 的示例提示词
...
```text
请使用 easyeda-api Skill 列出当前 EDA 中所有已打开的文档与画布。
```

```text
请在当前 PCB 工程中查找所有未连接的走线，并把结果以表格形式返回给我。
```
...
更多命令与开发规范请参阅仓库根目录的 [AGENTS.md](./AGENTS.md)。
